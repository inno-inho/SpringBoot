...
public interface MemoMapper {
    ...
    // 쿼리문이 실행 후에, 반환값은 Long으로, tbl_memo의 id중 최대값

    @Insert("insert into tbl_memo values(#{id}, #{text}, #{writer}, #{createAt})")      // #{}가 쿼리문이었다면 ?의 역할임
    ...
    @Delete("delete from tbl_memo where id=#{id}")      // 밑에 들어가있는 매개변수명이랑 다르면 안됨
    public int delete(@Param("id") Long MemoId);            // 혹시 다르다면 @Param으로 명시 해주어야함
