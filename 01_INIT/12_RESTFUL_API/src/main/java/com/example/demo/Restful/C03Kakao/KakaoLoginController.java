...
//    private String LOGOUT_REDIRECT_URI="http://localhost:8099/kakao";
    ...
    private KakaoFriendsResponse kakaoFriendsResponse;          // 카카오 친구 정보 담는거

    @GetMapping("/login")
    public String login(){
        log.info("GET /kakao/login...");
        return "redirect:https://kauth.kakao.com/oauth/authorize?client_id="+CLIENT_ID+"&redirect_uri="+REDIRECT_URI+"&response_type=code"; // GET(토큰내놔)
    }
...
//
    ...
    @GetMapping("/logout3")         // 동기처리임
                                    // 카카오 개발자 페이지의 카카오 로그인 고급의 리다이렉트 url 등록해야함
    ...
    public void message_friends(@PathVariable String message){
        ...
        for(int i = 0; i < list.size(); i++){
            array.add(list.get(i).getUuid());           // 친구의 정보를 JSONArray에 담는 중
        }
